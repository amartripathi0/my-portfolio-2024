...
function Skills() {
  ...
  const leftSlide = useTransform(scrollYProgress, [0, 1], ["0%", "-100vw"]);
  const rightSlide = useTransform(scrollYProgress, [0, 1], ["0%", "100vw"]);
  const opacity = useTransform(scrollYProgress, [0, 1], [1 , 0.2]);
  return (
    <motion.div
      id="skills"
      className="h-[calc(100vh-7rem)] pt-10 -mx-40 max-sm:pt-20 flex flex-col gap-14 relative border "
    >
      <motion.div
        initial={"initial"}
        whileInView={"inView"}
      ...
      <div className="text-xl max-sm:text-sm max-sm:px-2 text-justify  overflow-x-hidden flex-center h-full flex-col gap-4">
        <h2>{"// "}skills page is yet to be implemented {";)"}</h2>

        <motion.div
          ref={targetRef}
          style={{ translateX: leftSlide , opacity : opacity}}
          className=" flex gap-4 sticky bg-blue-400  p-10 "
        >
          {frontendSkills.map((skill) => (
            <div key={skill} className="h-20 aspect-square bg-purple-600">
              {skill}
            </div>
          ))}
        </motion.div>
        <motion.div
          ...
          className=" flex gap-4 sticky bg-blue-600  p-10 "
        >
